...
class LoginForm(forms.Form):
    ...
    # required默认输入框必须输入（required=True） widget=forms.PasswordInput密码输入框内字符隐藏为***
    # attrs字典加入css样式调整
    ...
    def clean(self):
        ...
        if user is None:
            ...
            raise forms.ValidationError('用户名或密码不正确') #　抛出异常
        else:
            self.cleaned_data['user'] = user
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean_nickname_new(self):
        ...
        if nickname_new == '':
            raise froms.ValidationError("新的昵称不能为空")
        return nickname_new
...
class BindEmailForm(forms.Form):
    ...
    def clean_email(self):
        email = self.cleaned_data['email']
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('该邮箱已经被绑定注册')
        return email
...
class ChangePasswordForm(forms.Form):
    ...
    def clean(self):
        ...
        if new_password != new_password_again or new_password == '':
            raise forms.ValidationError('两次输入的密码不一致') #　抛出异常
        return self.cleaned_data
    ...
    def clean_old_password(self):
        ...
        if not self.user.check_password(old_password):
            raise forms.ValidationError('旧的密码不正确') #　抛出异常
        return old_password
